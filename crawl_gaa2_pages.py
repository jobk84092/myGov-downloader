...
def main():
    ...
    # Write missing issues to a text file
    if missing_list:
        with open("missing_mygov_issues.txt", "w") as f:
            for fname in missing_list:
                f.write(fname + "\n")
        logger.info(f"Wrote {len(missing_list)} missing issues to missing_mygov_issues.txt")
